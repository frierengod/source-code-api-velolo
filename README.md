# Velolo API Wrapper (PHP) 🎬

Script PHP sederhana, super cepat, dan stabil untuk integrasi ke **API Aplikasi Velolo**. Sekarang kamu bisa memanggil data film pendek, daftar episode, ranking, dan pencarian video langsung dari aplikasi atau website PHP kamu tanpa ribet!

Repository ini adalah contoh cara menyambungkan aplikasimu ke API Velolo dengan menggunakan layanan dari [**StreamAPI**](https://streamapi.web.id), sang *Bridge API* andalan khusus buat platform video streaming.
...
- **Integrasi Gampang Banget**: Lupakan auth token yang ribet dan enkripsi aneh-aneh dari Velolo. Tinggal panggil URL REST API aja bareng Token kamu.
...
```bash
git clone https://github.com/username-kamu/velolo-api-php.git
cd velolo-api-php
php example.php
```

## 📖 Daftar Endpoint (Velolo)

Berikut adalah beberapa endpoint favorit yang sudah di-mapping oleh StreamAPI:

| **Method** | **Struktur Endpoint** | **Kegunaan** |
|:---:|---|---|
| GET | `/p/velolo/api/v1/foryou/{page}` | Ambil data drama *For You* yang lagi tren |
| GET | `/p/velolo/api/v1/new/{page}` | Tarik kumpulan list drama baru rilis |
| GET | `/p/velolo/api/v1/search/{keyword}/{page}` | Fitur pencarian judul & artis global |
| GET | `/p/velolo/api/v1/detail/{bookId}` | Lihat metadata, sinopsis, & cover drama |
| GET | `/p/velolo/api/v1/watch/{bookId}/{chapterId}` | Dapetin URL video/trailer/episode spesifik |

Ingin coba langsung di browser tanpa koding? Mampir aja ke fitur **[Sandbox Interaktif Velolo](https://streamapi.web.id/docs/velolo)** kami.
...
$url = "https://streamapi.web.id/p/velolo/api/v1/watch/{$bookId}/{$chapterId}";
...
echo "Respons Data dari API Velolo:\n" . $response;
?>
```
