<?php
/**
 * Contoh Script Bridge API Velolo via StreamAPI.web.id
 * ----------------------------------------------------
...
// Fungsi bantuan buat manggil endpoint Velolo di sistem Bridge
function callVeloloAPI($urlTujuan, $token) {
...
// UJI COBA KODE
// -----------------------------------------------------------------
$baseUrl = "https://streamapi.web.id/p/velolo/api/v1";

// Contoh 1: Fitur Cari Film Velolo
echo "\n=====================[ 1. Pencarian Velolo ]==================\n";
$kataKunci = urlencode("cinta");
$halaman = 1;
$urlCari = "{$baseUrl}/search/{$kataKunci}/{$halaman}?lang=in";

$hasilCari = callVeloloAPI($urlCari, $API_TOKEN);
if ($hasilCari) {
...
$hasilDetail = callVeloloAPI($urlDetail, $API_TOKEN);
if ($hasilDetail) {
...
$hasilNonton = callVeloloAPI($urlNonton, $API_TOKEN);
if ($hasilNonton) {
...
echo "▶▶▶ https://streamapi.web.id/docs/velolo ◀◀◀\n\n";
